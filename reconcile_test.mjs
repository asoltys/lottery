...
async function main() {
  ...
  for (const p of players) {
    ...
    if (claim > 0 && Math.abs(claim - acct.balance) > 2000) fail('covenant claim != balance after reconcile', `${claim} vs ${acct.balance}`);
  }
  ...
  console.log('\nPASS — after a win the lifecycle reconciled the covenant to the new balances; the winner withdrew their winnings cooperatively (was impossible before).');
  process.exit(0);
}
